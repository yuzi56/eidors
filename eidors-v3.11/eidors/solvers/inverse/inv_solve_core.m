...
% $Id: inv_solve_core.m 6503 2022-12-30 14:33:58Z aadler $
...
if nargin == 3
   test = strcmp(opt.meas_working{1}, {'apparent_resistivity','log_apparent_resistivity', 'log10_apparent_resitivity'});
   errstr= ['meas_working = ''' opt.meas_working{1} ''' not yet supported for difference solutions'];
   assert(all(~test), errstr);
...
%fprintf('TEST: map_meas(%s -> %s)\n', in, out);
   ...
   str = sprintf('map_data(%s -> %s)', in, out);
   unit_test_cmp(str, calc_val, expected,100*eps)
...
% -------------
% ADAPTED FROM
% Create simulation data $Id: inv_solve_core.m 6503 2022-12-30 14:33:58Z aadler $
