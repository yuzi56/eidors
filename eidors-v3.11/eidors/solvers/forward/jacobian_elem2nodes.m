...
% $Id: jacobian_elem2nodes.m 6422 2022-11-29 21:24:49Z aadler $
...
function e_d = calc_elem_from_node_image(EtoN, node_data); 
   ...
   A = [EtoN;mu*speye(n_elems)];
   B = [node_data;zeros(n_elems,1)];
   if 0 % previously used iterative
      [e_d, jnkflag] = lsqr(A,B, 1e-8,1000);
   else
      e_d = left_divide(A,B);
   end

return;
% Test code in nodal_jacobian01
