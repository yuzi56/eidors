...
% $Id: mdl_slice_mapper.m 6513 2022-12-30 19:20:08Z aadler $
...
function map = do_mdl_slice_mapper(fmdl, maptype)
    ...
    if isfield(fmdl.mdl_slice_mapper,'model_2d') && ...
          fmdl.mdl_slice_mapper.model_2d && ...
          size(fmdl.nodes,2) == 3
       NODE(3,:) = [];
    end
    fmdl.nodes = NODE';
...
function val = use_triangulation
   ver = eidors_obj('interpreter_version');
   val = ~ver.isoctave && ver.ver >= 9.013; % pointLocation was slow/buggy before
   
   
function ninterp_ptr = mdl_nodeinterp_mapper(NODE, ELEM, x, y)
   if use_triangulation
     ninterp_ptr = mdl_nodeinterp_mapper_triangulation( ...
        double(NODE), double(ELEM), x, y);
     return
   end
...
function EPTR= img_mapper2(NODE, ELEM, x, y );
  ...
  if ver.isoctave
    ...
    if use_triangulation 
       ...
       EPTR = img_mapper2_old(NODE, ELEM, x, y );
       return
    end
  end
  id(isnan(id)) = 0;
...
function EPTR= img_mapper3(NODE, ELEM, x, y );
  ...
  if ver.isoctave 
    ...
    %  id = tsearchn(NODE(:,use_nodes)', map(ELEM(:,use_elem))', [x(:),y(:),zeros(numel(x),1)]);
      ...
      if ver.ver <  9.013 % pointLocation was slow/buggy before
          EPTR = img_mapper3_old(NODE, ELEM, x, y);
          return
      end
      s = warning('off', 'MATLAB:triangulation:PtsNotInTriWarnId');
      TR = triangulation(double(ELEM'), double(NODE'));
      warning(s.state, 'MATLAB:triangulation:PtsNotInTriWarnId');
      pts = [x(:),y(:)]; pts(:,3) = 0;
...
function [NODE, ELEM] = level_model( fwd_model )
   ...
   if mdl_dim(fwd_model) ==2 || ... % 2D case
         isfield(fwd_model, 'mdl_slice_mapper') && ...
		 isfield(fwd_model.mdl_slice_mapper, 'model_2d') && ...
         fwd_model.mdl_slice_mapper.model_2d && ...
         ~isfield(fwd_model.mdl_slice_mapper, 'level')
